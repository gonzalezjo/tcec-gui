...
function setPgn(pgn)
{
  ...
  var base = Math.round(TC1[0].split('/')[1] / 60);
  formatTC = TC1[0].split('/')[0] + '/' + base + "'+" + TC1[1] + '", ';
  ...
  var title = "TCEC - Live Human Chess Broadcast";
  if (pgn.Moves.length > 0) {
...
function setInfoFromCurrentHeaders()
{
  ...
  $('.white-engine-elo').html(loadedPgn.Headers.WhiteElo);
  ...
  $('#white-engine-chessprogramming').attr('href', 'https://ratings.fide.com/card.phtml?event=' + loadedPgn.Headers.WhiteFide);
  ...
  $('.black-engine-elo').html(loadedPgn.Headers.BlackElo);
  $('.black-engine-name-full').html(header);
  var imgsrc = 'img/engines/' + name + '.jpg';
  $('#black-engine').attr('src', imgsrc);
  $('#black-engine').attr('alt', header);
  $('#black-engine-chessprogramming').attr('href', 'https://ratings.fide.com/card.phtml?event=' + loadedPgn.Headers.BlackFide);
}
...
function getEvalFromPly(ply)
{
  ...
  return {
    ...
    'depth': "",
    'tbhits': "",
    'timeleft': selectedMove.clk,
    'clk': selectedMove.clk
  };
}
...
function updateMoveValues(whiteToPlay, whiteEval, blackEval)
{
   ...
   if (!viewingActiveMove) 
   {
      ...
      $('.white-time-remaining').html(whiteEval.clk);
      $('.black-time-remaining').html(blackEval.clk);
   }
   else
   {
      if (whiteToPlay)
